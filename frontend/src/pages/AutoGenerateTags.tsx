...
import type { AutoGenerateTag } from "@/types";
import { toast } from "sonner";

import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";

export default function AutoGenerateTagsPage() {
    const [tags, setTags] = useState<AutoGenerateTag[]>([]);
    const [loading, setLoading] = useState(false);
    const [isEditing, setIsEditing] = useState(false);
    const [currentTag, setCurrentTag] = useState<Partial<AutoGenerateTag>>({});
    ...
    const handleSave = () => {
        ...
        const req = currentTag.id
            ? axios.put(`/api/auto-generate-tags/${currentTag.id}`, payload)
            : axios.post("/api/auto-generate-tags", payload);

        req.then(() => {
            setIsEditing(false);
            setCurrentTag({});
            fetchTags();
            toast.success(currentTag.id ? "标签已更新" : "标签已创建");
        }).catch(err => {
            console.error(err);
    ...
    const handleDelete = (id: number) => {
        toast("确定要删除这个标签吗？", {
            action: {
                label: "删除",
                onClick: () => {
                    axios.delete(`/api/auto-generate-tags/${id}`)
                        .then(() => {
                            fetchTags();
                            toast.success("标签已删除");
                        })
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold">剧情标签</h1>
                    <p className="mt-1 text-sm text-muted-foreground">
                        在这里维护题材标签规则。一键剧集或其他自动生成流程勾选哪些标签，就会把对应规则追加到 LLM 输入里。
                    </p>
                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Textarea
                                        ...
                                        autoResize={false}
                                        placeholder="直接写会注入到 LLM 的规则正文。建议使用短句和条目式描述。"
                                    />
                                    <p className="text-xs text-muted-foreground mt-1">
                                        勾选这个标签后，这段规则会直接追加到自动生成链路的系统提示词里。
                                    </p>
                                </div>
