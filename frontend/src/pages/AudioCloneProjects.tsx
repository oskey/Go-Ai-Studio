...
import type { AudioCloneProject } from "@/types";
...
export default function AudioCloneProjects() {
  const navigate = useNavigate();
  const [projects, setProjects] = useState<AudioCloneProject[]>([]);
  const [loading, setLoading] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingProject, setEditingProject] = useState<AudioCloneProject | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<AudioCloneProject | null>(null);
  ...
  const fetchProjects = async () => {
    setLoading(true);
    try {
      const res = await axios.get("/api/audio-clone-projects");
      setProjects(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error(err);
      toast.error("获取 LongChat 项目失败");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (project: AudioCloneProject) => {
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      const res = editingProject
        ? await axios.put(`/api/audio-clone-projects/${editingProject.id}`, payload)
        : await axios.post("/api/audio-clone-projects", payload);
      ...
      await fetchProjects();
      if (!editingProject) {
        navigate(`/audio-clone-projects/${res.data.id}`);
      }
    } catch (err: any) {
      console.error(err);
  ...
  const deleteProject = async () => {
    ...
    try {
      await axios.delete(`/api/audio-clone-projects/${deleteTarget.id}`);
      toast.success("项目已删除");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">LongChat</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            音频复制模板：上传角色参考音频和原始文本，再用 {"{角色名}台词"} 格式批量生成配音。
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="space-y-3">
        {projects.map((project) => (
          <div key={project.id} className="rounded-xl border bg-card p-4 shadow-sm">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate(`/audio-clone-projects/${project.id}`)}
              ...
              <button onClick={() => navigate(`/audio-clone-projects/${project.id}`)} className="min-w-0 flex-1 text-left">
        ...
        {!loading && projects.length === 0 ? (
          <div className="rounded-xl border border-dashed p-12 text-center text-muted-foreground">还没有 LongChat 项目。</div>
        ) : null}
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editingProject ? "编辑 LongChat 项目" : "新建 LongChat 项目"}</DialogTitle>
            <DialogDescription>项目创建后，在详情页维护角色音频和批量脚本。</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium">项目名称</label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：张继先配音测试" />
            </div>
            <div>
              <label className="text-sm font-medium">项目文件名</label>
              <Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="例如：zhang_jixian_voice" disabled={!!editingProject} />
            </div>
            <div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>删除 LongChat 项目？</AlertDialogTitle>
